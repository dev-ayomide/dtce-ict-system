  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <main className="px-4 md:px-8 lg:px-16 pt-20 pb-4 md:pb-8 max-w-7xl mx-auto">
        {/* Page Title */}
        <h1 className="text-2xl md:text-3xl font-semibold text-black mb-6">Attendance</h1>

        {/* Check In Streak Section */}
        <div className="bg-[#f8f8f8] rounded-[15px] p-6 mb-6">
          {/* Header Row */}
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-lg font-semibold text-black">Check In Streak</h2>
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-lg font-semibold text-[#0153FD]">4 Days</span>
            </div>
          </div>
          ...
          <div className="flex justify-center gap-4 mb-6">
            {weekDays.map((day, index) => (
              <div key={`${day.label}-${index}`} className="flex flex-col items-center gap-2">
                <span className="text-sm font-semibold text-black">{day.label}</span>
                <div className={`w-4 h-4 rounded-sm ${day.checked ? 'bg-[#0153FD]' : 'bg-[#d9d9d9]'}`}></div>
              </div>
            ))}
          </div>
          ...
          <div className="flex justify-center gap-4">
            ...
            <button
              ...
              className="bg-white border border-[#0153FD] text-[#0153FD] px-6 py-2 rounded-lg text-sm font-medium hover:bg-blue-50 transition-colors"
            >
              Clock Out
            </button>
          </div>
        ...
        <div>
          <h2 className="text-2xl md:text-3xl font-semibold text-black mb-6">Attendance History</h2>
          
          {/* Filters */}
          <div className="bg-[#f8f8f8] rounded-[15px] p-6 mb-12">
            <div className="flex flex-col md:flex-row gap-4 md:gap-6 items-start">
              {/* Date Range Filter */}
              <div className="flex flex-col gap-3 flex-1 w-full">
                <label className="text-base font-semibold text-black">Date Range</label>
                <div className="relative w-full">
                  <div className="bg-[rgba(0,0,0,0.02)] border border-[#e0e0e0] rounded-xl h-14 flex items-center justify-between px-5 w-full">
                    <span className="text-sm text-[#878787]">All time</span>
              ...
              <div className="flex flex-col gap-3 flex-1 w-full">
                <label className="text-base font-semibold text-black">Attendance type</label>
                <div className="relative w-full">
                  <div className="bg-[rgba(0,0,0,0.02)] border border-[#e0e0e0] rounded-xl h-14 flex items-center justify-between px-5 w-full">
                    <span className="text-sm text-[#878787]">All</span>
                    <svg className="w-5 h-5 text-[#878787] flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div key={record.id}>
                <div className="flex items-start gap-[10px] py-4">
                  ...
                  <div className="flex-1">
                    <p className="text-base font-semibold text-[#020202] mb-1">{record.date}</p>
                    <p className="text-sm text-[#4a4a4a]">{record.type} {record.time}</p>
                  </div>
                </div>
          ...
          <div className="flex items-center justify-center gap-[17px]">
            <button className="border border-[#0153FD] px-4 py-2 rounded-full text-xs font-semibold text-black hover:bg-blue-50 transition-colors">
              &lt;
            </button>
            
            <button className="bg-[#f6f6f6] px-4 py-2 rounded-full text-xs font-semibold text-black hover:bg-gray-200 transition-colors">
              1
            </button>
            <button className="bg-[#f6f6f6] px-4 py-2 rounded-full text-xs font-semibold text-black hover:bg-gray-200 transition-colors">
              2
